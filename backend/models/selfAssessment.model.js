...
// Indexes
selfAssessmentSchema.index({ performanceCycle: 1, employee: 1 }, { unique: true });
selfAssessmentSchema.index({ employee: 1, status: 1 });
selfAssessmentSchema.index({ submittedAt: -1 });

// Pre-save hook to calculate weighted score
selfAssessmentSchema.pre('save', async function(next) {
